...
function OrderListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrderInterface[]>(
    ...
    () =>
      getOrders({
        relations: ['user_order_delivery_manager_idTouser', 'user_order_delivery_personnel_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_order_delivery_manager_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_order_delivery_personnel_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_order_delivery_manager_idTouser?.id}`}>
                          {record.user_order_delivery_manager_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_order_delivery_personnel_idTouser?.id}`}>
                          {record.user_order_delivery_personnel_idTouser?.email}
                        </Link>
                      </Td>
                    )}
